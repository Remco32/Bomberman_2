...
/**
 * Created by joseph on 15/05/2017.
 */
public class SoftMaxNNAIFullInput extends AIHandler{
    ...
    public SoftMaxNNAIFullInput(GameWorld world, int manIndex, NNSettings setting, GameSettings gSet) {
    ...
    public int SoftMax(double[] Qvalues){
        int move=0;
        ...
        double Qsum=0;
        Arrays.sort(Qclone);
        double maxQ=Math.abs(Qclone[Qvalues.length-1]);
        if(maxQ==0)maxQ=0.001;
        for(double val:Qvalues){
            Qsum+=Math.exp(val/maxQ);
        }
        ...
        for(int idx=0;idx<Qvalues.length;idx++){
            probabilitys[idx] = Math.exp(Qvalues[idx]/maxQ)/Qsum;
        }
        ...
        for(int idx=0;idx<Qvalues.length;idx++){
            cumsumprobability[idx] = cumsum + probabilitys[idx]*100;
